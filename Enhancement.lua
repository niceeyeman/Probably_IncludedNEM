...
if TIER16_CHECK ~= true then
	...
	for i=1, 19 do
		if GetInventoryItemID("player", i) ~= nil then
			for j=1, 5 do
				if GetItemInfo(GetInventoryItemID("player", i)) == GetItemInfo(NEEnhanceT16[j]) then
					NET_16 = NET_16 + 1
				end
			end
		end
...
if TIER15_CHECK ~= true then
	...
	for i=1, 19 do
		if GetInventoryItemID("player", i) ~= nil then
			for j=1, 5 do
				if GetItemInfo(GetInventoryItemID("player", i)) == GetItemInfo(nemEnhanceT15[j]) then
					NET_15 = NET_15 + 1
				end
			end
		end
...
ProbablyEngine.rotation.register_custom(263, "NEM Group", 
{
-- *** NEM Standards ***		
